...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($user_name) && !empty($password) && !empty($email) && !empty($age) && !empty($gender) &&  !is_numeric($user_name)) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $user_id = random_num(20);
        $query = "INSERT INTO bit (user_id, user_name, password, email, age, gender) VALUES ('$user_id', '$user_name', '$hashed_password', '$email', '$age', '$gender')";
